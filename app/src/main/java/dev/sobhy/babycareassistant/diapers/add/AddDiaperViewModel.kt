...
class AddDiaperViewModel @Inject constructor(
        ...
        when(event){
            ...
            is AddDiaperUiEvent.NumberOfDiaperChanged -> {
                val newFieldsVal = if (event.number.isBlank()) emptyList() else List(event.number.toInt()){index ->
                    _addDiaperState.value.timesOfDiaperChange.getOrNull(index) ?: LocalTime.of(6, 0)
                }
                _addDiaperState.update {
                    it.copy(
    ...
    private fun addOrUpdateDiaper(id: String?){
        val breastFeedingState = addDiaperState.value
        val errors = mapOf(
            "dateError" to breastFeedingState.diaperDate.isEqual(LocalDate.of(1000, 1, 1)),
            "numberOfDiaperChangeError" to breastFeedingState.numOfDiaperChange.isBlank(),
        ).filterValues { it }
        ...
        viewModelScope.launch(Dispatchers.IO) {
            val diaper = Diapers(
                id = id ?: "",
                date = breastFeedingState.diaperDate.toString(),
                day = breastFeedingState.diaperDay,
                numberOfDiapersChange = breastFeedingState.numOfDiaperChange.toInt(),
                timesOfDiapersChange = breastFeedingState.timesOfDiaperChange.map { it.toString() }
            )
